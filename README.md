...
- **Clients** (`/clients`) — details-first view of every lead whose status is Booked (no separate schema — same `Lead` records as `/leads`, just filtered and shown with contact/email/phone/website/address inline on the card). Click through to `/leads/[id]` to edit.
- **Add Lead / Add Task** (floating **+** button, bottom-right on every page — click it to toggle a small menu with both options) — Add Lead opens `/leads/new`, business info (name, contact, email, phone, website, trade, full address) plus optional leak notes/internal notes, which Gmail account to send from (if more than one is connected), **and the full email sequence right there**: an Initial Email plus as many Follow-up tabs as you add via "+ Add follow-up" before you've even saved the lead, each with its own body, a send date/time/timezone, and an "Include a subject line" checkbox. Follow-ups also get a "Send as" choice (reply in the same thread vs. a separate new email) and a pipeline condition (see below). There's no inline "Add Lead" button anywhere else — the floating button is the only entry point, by design.
...
- **Outreach Calendar** (`/calendar`, was `/today`) — every unsent, pipeline-eligible email step, always judged and displayed in Pakistan time regardless of the lead's own timezone. **Today** view is the original detailed copy-paste-ready snippet cards (To, Subject, Body, each with a copy icon). **Week/Month** views (tabs, `?view=`) render as a real Google-Calendar-shaped grid (`src/components/CalendarGrid.tsx`) — a true Sun–Sat week or full month, items as small pills you click through to the lead.
- **Tasks** (`/tasks`) — plain to-dos: title, short description, and an optional client (`Lead`) it's for. Same Today (detailed cards, with an Overdue section) / Week / Month (grid) views as the other calendars, plus a fourth **Done** tab listing everything marked complete — "Mark done" doesn't delete a task, it moves it there. Add one via the floating **+** menu → Add Task (`/tasks/new`). Top-right has **Import CSV** / **Export CSV** (`/api/tasks/import`, `/api/tasks/export` — columns: `title,description,client,dueDate,dueTime,completed`; `client` is matched to an existing lead by business name, case-insensitive, and left blank/general if no match).
...
### Slack notifications (action-needed digest)

`.github/workflows/notify.yml` runs every 3 hours (unlike scheduled-send, this one is **not** paused — it's read-only, it never sends anything itself) and hits `/api/cron/notify`, which posts a Slack message listing anything due: unsent scheduled emails, unmarked engagement check-ins, and open tasks with a due date (today or overdue, Pakistan time — see `src/lib/notifications.ts`). It sends nothing if the queue is empty. Needs `SLACK_WEBHOOK_URL` set on Vercel (an Incoming Webhook URL from a Slack app — see `.env.example`); without it, `sendSlackMessage` in `src/lib/slack.ts` just no-ops.

### 7-day social engagement checklist

Checking a platform (Instagram/Facebook/LinkedIn) on a lead's Business details seeds 7 `EngagementDay` rows for it (days 1–7, dated sequentially starting the day it was checked) — see `seedEngagementDays` in `src/lib/actions.ts`. Re-saving the lead form never re-seeds or resets a platform that's already there, only genuinely new platforms. Marking a day done (or adding a note) is per-day, from `/engagement/[id]`.

## Data model (`prisma/schema.prisma`)

- `Lead` — one business being pursued: contact info, trade, `secondaryEmails` (extra addresses beyond the primary `email`, reference/display only — sending and inbox scoping still use `email`), `socialPlatforms` (which of Instagram/Facebook/LinkedIn it's active on — checking one seeds an `EngagementDay` checklist), full `address` (no separate city/state/source fields), status, leak notes, and `sendAccountId` (which connected `EmailAccount` its sequence sends from; null = use the default account).
...
- `Task` — a to-do: `title`, `description`, optional `leadId` ("which client this is for"), `dueDate`/`dueTime` (plain strings, same convention as `EmailStepRecord`), `completedAt`. No fixed relation to anything else — plain standalone tasks.
