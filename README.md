...
- **Tasks** (`/tasks`) — plain to-dos: title, short description, an optional client (`Lead`) it's for, and a due **date** (date only, no time of day). Tabs: **Overdue** (first, red-tinted cards, red hover) / **Today** (detailed cards) / **Week** / **Month** (grid) / **Done** — "Mark done" doesn't delete a task, it moves it there. Add one via the floating **+** menu → Add Task (`/tasks/new`); every card has an **Edit** link → `/tasks/[id]/edit` for single-task edits. Top-right has **Bulk Edit** / **Import CSV** / **Export CSV**:
