...
- **Today's Outreach** (`/today`) — every unsent, pipeline-eligible email step scheduled for *today* (checked against each step's own timezone, not the server's), sorted by send time. Each one shows as a copy-paste-ready snippet box (Subject, Body) with a copy icon in the top-right corner — built for manually sending on schedule while automatic sending is paused (see below).
